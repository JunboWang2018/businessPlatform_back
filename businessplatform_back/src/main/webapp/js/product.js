...
function addAuctionRecord(prodNumber) {
    var auctionPrice = $("#auction_price").val();
    var minAuctionPrice = $("#min_auction_price").val();

    if (auctionPrice < minAuctionPrice) {
        alert("输入价格不能低于竞拍价格，请重新输入");
        return;
    }
    $.ajax({
        type : "get",
